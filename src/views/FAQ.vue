  ...
  <div class="home">
    ...
    <div class="faq-container">
      ...
      <table class="faq-table">
        ...
        <tbody>
          <template v-for="(faq, index) in paginatedFAQ">
      ...
      <div class="pagination">
        <button @click="prePage" :disabled="currentPage === 1">이전</button>
...
// 현재 페이지에 표시할 F&Q 목록 (computed() 사용)
const paginatedFAQ = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  // console.log(start);
  const result = faqs.value.slice(start, start + itemsPerPage); // slice는 끝 숫자의 바로 앞까지 보여주는 것
  return result;
});
